<template>
  <div>
    <h1>https://sample-support-kuroco.r-cms.jp/management/ のフォームid=14</h1>
...
<script>
const FORM_ID = 14 // 作成したフォーム定義のID

export default {
  async asyncData({ $axios }) {
    const response = await $axios.$get(
      `https://sample-support-kuroco.a.kuroco.app/rcms-api/11/form/${FORM_ID}`
    )
    return {
    ...
    async handleOnSubmit(e) {
      ...
      try {
        ...
        const { id } = await this.$axios.$post(
          `https://sample-support-kuroco.a.kuroco.app/rcms-api/11/form?id=${FORM_ID}`,
          body
        )
        this.error = null
        this.submittedId = id
